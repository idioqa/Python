...
if(item_1 < comp_item): print("Переменная item_1 меньше", comp_item)
 ...
 #  item_2 = ”, item_2), иначе, вывести в консоль (“Переменная item_2 = ”, item_3)
if(item_2): print("Переменная item_2 = ", item_2)
else: ("Переменная item_2 = ", item_3)
...
currency_convertor = item_2
 ...
 #30. Сделать if в котором будет условие: если currency_convertor, то выполнять
 #  следующие шаги задания, иначе, вывести в консоль (“Переменная currency_convertor = ”, item_3)
...
def calc_result(amount, rate):
    currency_result = 0
    currency_result = round(amount / rate, 2)
    return currency_result

def print_result(amount, tar_item, res_item, result):
    print(amount, tar_item, " = ",  result, res_item)
...
while(currency_convertor):
    ...
    if(target_currency == 'q'): break

    print("Write amount of your currency")
    inp = input()
    if(inp.isdigit()) : target_currency_amount = int(inp)
    else: 
    ...
    if(currency_convertor): 
        ...
        if(target_currency == 'eur'): 
            rate = usd_eur_rate
            target_item = eur_item

        elif(target_currency == 'uah'): 
            rate = usd_uah_rate
            target_item = uah_item

        elif (target_currency == 'chf'):
            rate = usd_chf_rate
            target_item = chf_item

        elif (target_currency == 'rub'):
            rate = usd_rub_rate
            target_item = rub_item

        elif (target_currency == 'byn'):
            rate = usd_byn_rate
            target_item = byn_item

        else: 
            print("Uknown currency")
            continue

        print_result(target_currency_amount, target_item, currency_convertor, calc_result(target_currency_amount, rate))

...
#2 оздать переменную range_count со значением 10
...
while count < range_count:
  print("Step =", count)
  count += 1
  if count == 3: print("Step =", count, "If body")
...
for item in range(0, 30):
  print("Step =", item)
  if(item == 5): print("Item =", item)
  elif(item == 10): print("Item =", item)
  elif(item < 4): print("Item <", item)
  elif(item >= 27): print("Item >=", item)
...
for item in range(0, range_count + 1):
  ...
  if item == 7: 
    ...
    for inner_item in range(0, item):
      ...
      if inner_item == 5: inner_count = inner_item
    print("-- inner_count =", inner_count)

#13.делать цикл for c переменной item который будет работать пока счётчик range досчитает от 0 до 20
#Тело цикла:
